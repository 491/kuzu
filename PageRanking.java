...
public class PageRanking {
	...
	{    
		...
		try {
			...
			ResultSet rs1 = stmt1.executeQuery("select id,Outgoing from search4");
			...
			while (rs1.next()) 
			{
				...
				if(!StringUtils.isNullOrEmpty(outgoings)){
					...
					while (st.hasMoreTokens()) {
						...
						ResultSet rs2 = stmt2.executeQuery("select id from search4 where link = '"+each+"'");
	...
	{    
		...
		try {
			Statement stmt1 = (Statement) db.conn.createStatement();
			ResultSet rs1 = stmt1.executeQuery("select count(id) from search4");
	...
	public static void main(String args[]) throws SQLException
	{
		...
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) {
				pr.AMatrix[i][j] = (1-pr.p) * pr.AMatrix[i][j] + pr.BMatrix;
			}
		}
		...
		for (i = 0;  i< n; i++) {
			sum=0;
			for (j = 0;  j< n; j++) {
				sum += pr.VVector[j] * pr.AMatrix[i][j];
			}
		...
		while(pr.notEqual(pr.TempMatrix, pr.ResultMatrix) || count==0){ 
			...
			for (i = 0;  i< n; i++) {
				sum=0;
				for (j = 0;  j< n; j++) {
					sum += pr.ResultMatrix[j] * pr.AMatrix[i][j];
				}
				pr.ResultMatrix[i] = sum;
				//for (j = 0;  j< n; j++) {
		...
		for (i = 0; i < n; i++) {
			...
			stmt1.executeUpdate("update search4 set Popularity = "+pr.ResultMatrix[i]+" where id="+(i+1));
		}
	}   
